...
export default function ShopPage() {
  ...
  useEffect(() => {
      const fetchProducts = async () => {
        try {
          const response = await fetch("https://ecommerce.ahmedgamaldev.com/api/products?limit=32&page=1");
  ...
  const handlesearch = async (term) => {
    try {
      const response = await fetch(
        `https://ecommerce.ahmedgamaldev.com/api/products/search?search=${encodeURIComponent(term)}`
      );
      const result = await response.json();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-blue-50 to-purple-50 p-8">
      ...
      <div className="max-w-10xl mx-auto">
        ...
        {/* Products Grid */}
        {products != null ?(<motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <span className="text-center">لا توجد منتجات بعد</span>
        </div>)}
      </div>
    </div>
  );
